...
$messages = Array();
$published = false;
if (isset($_POST['date_depart']) && isset($_POST['h_depart']) && isset($_POST['min_depart']) && isset($_POST['date_arrivee']) && isset($_POST['h_arrivee']) && isset($_POST['min_arrivee']) && isset($_POST['destination']) && isset($_POST['message'])) {
	...
	if (empty($messages)) {
		$ajoutAnnonce = $bdd->prepare('INSERT INTO `annonces` SET auteur=?,horaire_depart=?,horaire_arrivee=?,destination=?,message=?');
			...
			if ($published) {
				...
				<form method="post" action="publier-annonce.php">
					<fieldset>
						...
						<p><label><span class="form-input">Heure :</span><span class="form-value"><input type="text" name="h_depart" value="<?php print_value('h_depart'); ?>" size="1" required />h<input type="text" name="min_depart" value="<?php print_value('min_depart'); ?>" size="1" /></span></label></p>
					</fieldset>
					<fieldset>
